...
module EgoiRubyClient
  class AutomationsApi
    ...
    # @param automation_id [Integer] ID of the Automation
    ...
    def delete_automation_with_http_info(automation_id, opts = {})
      ...
      if @api_client.config.client_side_validation && automation_id < 1
        fail ArgumentError, 'invalid value for "automation_id" when calling AutomationsApi.delete_automation, must be greater than or equal to 1.'
      end

      # resource path
      local_var_path = '/automations/{automation_id}'.sub('{' + 'automation_id' + '}', CGI.escape(automation_id.to_s))
      ...
      new_options = opts.merge(
        :operation => :"AutomationsApi.delete_automation",
    ...
    def get_all_actions(opts = {})
      data, _status_code, _headers = get_all_actions_with_http_info(opts)
      data
    end
    ...
    # @option opts [Integer] :offset Element offset (starting at zero for the first element)
    # @option opts [Integer] :limit Number of items to return (default to 10)
    # @return [Array<(AutomationActionsCollection, Integer, Hash)>] AutomationActionsCollection data, response status code and response headers
    def get_all_actions_with_http_info(opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: AutomationsApi.get_all_actions ...'
      end
      if @api_client.config.client_side_validation && !opts[:'offset'].nil? && opts[:'offset'] < 0
        fail ArgumentError, 'invalid value for "opts[:"offset"]" when calling AutomationsApi.get_all_actions, must be greater than or equal to 0.'
      end

      if @api_client.config.client_side_validation && !opts[:'limit'].nil? && opts[:'limit'] > 1000
        fail ArgumentError, 'invalid value for "opts[:"limit"]" when calling AutomationsApi.get_all_actions, must be smaller than or equal to 1000.'
      end

      if @api_client.config.client_side_validation && !opts[:'limit'].nil? && opts[:'limit'] < 1
        fail ArgumentError, 'invalid value for "opts[:"limit"]" when calling AutomationsApi.get_all_actions, must be greater than or equal to 1.'
      end

      # resource path
      local_var_path = '/automations/{automation_id}/actions'
      ...
      # return_type
      return_type = opts[:debug_return_type] || 'AutomationActionsCollection'

      # auth_names
      auth_names = opts[:debug_auth_names] || ['Apikey']

      new_options = opts.merge(
        :operation => :"AutomationsApi.get_all_actions",
        :header_params => header_params,
        :query_params => query_params,
        :form_params => form_params,
        :body => post_body,
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: AutomationsApi#get_all_actions\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end
    ...
    # @option opts [String] :order Type of order (default to 'desc')
    # @option opts [String] :order_by Reference attribute to order automations (default to 'automation_id')
    # @return [Array<(AutomationCollection, Integer, Hash)>] AutomationCollection data, response status code and response headers
    def get_all_automations_with_http_info(opts = {})
      ...
      if @api_client.config.client_side_validation && !opts[:'list_id'].nil? && opts[:'list_id'] < 1
        fail ArgumentError, 'invalid value for "opts[:"list_id"]" when calling AutomationsApi.get_all_automations, must be greater than or equal to 1.'
      end
      ...
      if @api_client.config.client_side_validation && !opts[:'limit'].nil? && opts[:'limit'] > 1000
        fail ArgumentError, 'invalid value for "opts[:"limit"]" when calling AutomationsApi.get_all_automations, must be smaller than or equal to 1000.'
      end
      ...
      allowable_values = ["automation_id", "title", "list_id", "created_by"]
      ...
      # return_type
      return_type = opts[:debug_return_type] || 'AutomationCollection'

      # auth_names
      auth_names = opts[:debug_auth_names] || ['Apikey']

      new_options = opts.merge(
        :operation => :"AutomationsApi.get_all_automations",
        :header_params => header_params,
        :query_params => query_params,
        :form_params => form_params,
        :body => post_body,
