...
module EgoiRubyClient
  class CampaignsApi
    ...
    def delete_campaigns_with_http_info(campaign_hash, opts = {})
      ...
      # resource path
      local_var_path = '/campaigns/{campaign_hash}'.sub('{' + 'campaign_hash' + '}', campaign_hash.to_s)
    ...
    # @return [Array<(CampaignsCollection, Fixnum, Hash)>] CampaignsCollection data, response status code and response headers
    def get_all_campaigns_with_http_info(opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: CampaignsApi.get_all_campaigns ...'
      end
      if @api_client.config.client_side_validation && opts[:'channel'] && !['email', 'push', 'sms', 'smart_sms', 'voice', 'web_push'].include?(opts[:'channel'])
        fail ArgumentError, 'invalid value for "channel", must be one of email, push, sms, smart_sms, voice, web_push'
      end
      if @api_client.config.client_side_validation && opts[:'status'] && !['draft', 'canceled', 'queued', 'paused', 'sending', 'sent'].include?(opts[:'status'])
        fail ArgumentError, 'invalid value for "status", must be one of draft, canceled, queued, paused, sending, sent'
      end
      ...
      if @api_client.config.client_side_validation && opts[:'order_by'] && !['list_id', 'internal_name', 'created_by', 'group_id', 'created'].include?(opts[:'order_by'])
        fail ArgumentError, 'invalid value for "order_by", must be one of list_id, internal_name, created_by, group_id, created'
      end
      # resource path
      ...
      data, status_code, headers = @api_client.call_api(:GET, local_var_path,
        ...
        :body => post_body,
        :auth_names => auth_names,
        :return_type => 'CampaignsCollection')
      if @api_client.config.debugging
