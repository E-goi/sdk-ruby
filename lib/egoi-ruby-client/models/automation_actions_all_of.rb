  ...
  class AutomationActionsAllOf
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `EgoiRubyClient::AutomationActionsAllOf` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `EgoiRubyClient::AutomationActionsAllOf`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@action_id.nil? && @action_id < 1
        invalid_properties.push('invalid value for "action_id", must be greater than or equal to 1.')
      end
    ...
    def valid?
      return false if !@action_id.nil? && @action_id < 1
    ...
    def action_id=(action_id)
      if !action_id.nil? && action_id < 1
        fail ArgumentError, 'invalid value for "action_id", must be greater than or equal to 1.'
      end
    ...
    def hash
      [action_id, title, action, parent_id, created_by, created, updated].hash
    end
