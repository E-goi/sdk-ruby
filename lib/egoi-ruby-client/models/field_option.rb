  ...
  class FieldOption
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `EgoiRubyClient::FieldOption` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `EgoiRubyClient::FieldOption`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@option_id.nil? && @option_id < 1
        invalid_properties.push('invalid value for "option_id", must be greater than or equal to 1.')
      end
    ...
    def valid?
      return false if !@option_id.nil? && @option_id < 1
      true
    end
    ...
    def option_id=(option_id)
      if !option_id.nil? && option_id < 1
        fail ArgumentError, 'invalid value for "option_id", must be greater than or equal to 1.'
      end
    ...
    def hash
      [option_id, en, pt, br, es, de, hu, fr].hash
    end
