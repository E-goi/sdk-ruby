  ...
  class StartAutomationRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `EgoiRubyClient::StartAutomationRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `EgoiRubyClient::StartAutomationRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      return false if @automation_id < 1
      return false if @action_id.nil?
      return false if @action_id < 1
    ...
    def automation_id=(automation_id)
      if automation_id.nil?
        fail ArgumentError, 'automation_id cannot be nil'
      end
    ...
    def hash
      [automation_id, action_id, contacts].hash
    end
