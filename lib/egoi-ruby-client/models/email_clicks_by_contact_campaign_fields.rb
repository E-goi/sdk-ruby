  ...
  class EmailClicksByContactCampaignFields
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `EgoiRubyClient::EmailClicksByContactCampaignFields` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `EgoiRubyClient::EmailClicksByContactCampaignFields`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @campaign_hash.nil?
        invalid_properties.push('invalid value for "campaign_hash", campaign_hash cannot be nil.')
      end
    ...
    def hash
      [internal_name, campaign_hash, url, send_date, group, channel, type, sender, city, country, region, program, os].hash
    end
